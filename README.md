...
- [x] Rulebase security audit — `panos-audit audit` + the check framework in `audit.py`; first check implemented (any/any allow rules), remaining checks specced in [AUDIT-CHECKS.md](AUDIT-CHECKS.md) (offline-tested)
- [ ] Remaining audit checks per [AUDIT-CHECKS.md](AUDIT-CHECKS.md): logging-disabled, disabled-rule-hygiene, shadowed-rule, broad-service-object, mgmt-plane-settings (the last gated on a real config export)
